...
void CamImpl::Start() {
    ...
    current_thread_->PostTask(webrtc::ToQueuedTask([=] {
        ...
        do {
            if (has_start_) {
                RTC_LOG(LS_WARNING) << "CamImple already start,ignore";
                break;
            }
            ...
            // 获取一个最佳匹配的摄像头能力(参数可使用cam_impl中的Setup方法)
...
//对视频帧进行处理，包括帧率计算、数据拷贝、时间戳处理等操作，并将处理后的帧数据通过异步任务分发给消费者（consumer）
