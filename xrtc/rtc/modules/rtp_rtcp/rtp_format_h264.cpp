...
namespace xrtc
{
    ...
    RtpPacketizerH264::RtpPacketizerH264(
        ...
        const RtpPacketizer::Config &config) : config_(config)
    {
        // 查找所有NALU并存储到输入片段队列
    ...
    bool RtpPacketizerH264::NextPacket(RtpPacketToSend *rtp_packet)
    {
        ...
        // 获取队列前端的包 (使用 const 引用可能更高效)
        const PacketUnit &packet = packets_.front();
        // 暂存 last_fragment 状态，因为 packet 引用会在 pop 后失效
        bool is_last_fragment_in_nalu = packet.last_fragment;

        if (packet.first_fragment && packet.last_fragment && !packet.aggregated)
        { // Corrected condition: single NALU packet
            // 单个NALU包：直接复制整个NALU
            ...
            {
                packets_.pop(); // 移除当前处理失败的包
                --num_packets_left_;
                // RTC_LOG(LS_ERROR) << "Failed to allocate payload for single NALU packet.";
                return false;
            }
            // 从队列移除已处理的包
            packets_.pop();
        ...
        {
            // STAP-A聚合包：调用聚合包处理函数，传入当前 packet
            NextAggregatedPacket(rtp_packet, packet);
            // NextAggregatedPacket 内部会处理 packets_.pop()
        }
        else
        {
            // FU-A分片包：调用分片包处理函数，传入当前 packet
            NextFragmentPacket(rtp_packet, packet);
            // 从队列移除已处理的包
            packets_.pop();
        }
        ...
        // 更新剩余包数量
        --num_packets_left_;
        // 设置RTP包的标记位: 当这个包是其NALU的最后一个分片/部分，并且处理完后队列为空时，设置标记
        rtp_packet->SetMarker(is_last_fragment_in_nalu && packets_.empty());
    ...
    // 处理下一个聚合包（STAP-A）
    void RtpPacketizerH264::NextAggregatedPacket(RtpPacketToSend *rtp_packet, const PacketUnit &first_packet)
    {
        // 分配RTP包负载空间 (需要估算或使用最大可能大小)
        // 注意：简单的 FreeCapacity 可能不足以容纳所有聚合的NALU
        // 更健壮的方法是先计算总大小或使用足够大的缓冲区
        size_t rtp_packet_capacity = rtp_packet->FreeCapacity();
        uint8_t *buffer = rtp_packet->AllocatePayload(rtp_packet_capacity);
        if (!buffer)
        {
            // RTC_LOG(LS_ERROR) << "Failed to allocate payload for STAP-A packet.";
            // 需要处理 first_packet 对应的 pop 操作吗？NextPacket 会处理 num_packets_left_
            // 但 first_packet 对应的 pop 需要在此处或 NextPacket 中处理
            // packets_.pop(); // 从队列移除 first_packet (如果 NextPacket 不处理 STAP-A 的 pop)
            // 这里暂时不 pop，假设 NextPacket 处理失败时的 pop
            return;
        }

        // 写入STAP-A头部（使用第一个 NALU 的信息）
        buffer[0] = (first_packet.header & (kFBit | kNriMask)) | NaluType::kStapA;
        size_t index = kNaluHeaderSize;

        // 处理第一个 packet (传入的 first_packet)
        rtc::ArrayView<const uint8_t> fragment = first_packet.source_fragment;
        if (index + kLengthFieldSize + fragment.size() > rtp_packet_capacity)
        {
            // RTC_LOG(LS_ERROR) << "Buffer too small for first NALU in STAP-A";
            // packets_.pop(); // 移除 first_packet
            rtp_packet->SetPayloadSize(0); // Or handle error appropriately
            return;
        }
        ...
        // 从队列移除 first_packet (它已经被处理了)
        packets_.pop(); // **** IMPORTANT: Pop the first packet ****
        // --num_packets_left_; // NextPacket 会处理计数器

        bool is_last_fragment_in_stap = first_packet.last_fragment;

        // 处理队列中剩余的属于此 STAP-A 包的 NALU
        while (!is_last_fragment_in_stap && !packets_.empty())
        {
            const PacketUnit &next_packet = packets_.front();
            // 检查下一个包是否确实是聚合包的一部分
            if (!next_packet.aggregated)
            {
                // RTC_LOG(LS_WARNING) << "Expected aggregated packet but got something else.";
                break;
            }

            fragment = next_packet.source_fragment;
            size_t needed = kLengthFieldSize + fragment.size();
            if (index + needed > rtp_packet_capacity)
            {
                // RTC_LOG(LS_ERROR) << "STAP-A buffer overflow while aggregating.";
                // 无法添加更多 NALU，结束当前 STAP-A 包
                break;
            }
            ...
            is_last_fragment_in_stap = next_packet.last_fragment;
    ...
    // 处理下一个分片包（FU-A）
    void RtpPacketizerH264::NextFragmentPacket(RtpPacketToSend *rtp_packet, const PacketUnit &packet)
    {
        ...
        size_t payload_size = kFuAHeaderSize + fragment.size();
        uint8_t *buffer = rtp_packet->AllocatePayload(payload_size);
        if (!buffer)
        {
            // RTC_LOG(LS_ERROR) << "Failed to allocate payload for FU-A packet.";
            // packets_.pop(); // NextPacket 会处理 pop 和计数器
            return; // 让 NextPacket 处理后续逻辑
        }
        ...
        // packets_.pop(); // 在 NextPacket 中处理
        // if (packet.last_fragment) {
        //     input_fragments_.pop_front(); // 移除对应的输入片段？需要确认 input_fragments_ 的管理逻辑
        // }
    }
