    ...
    encode_thread_ = new std::thread([=]() {
        ...
        while (running_) {
...

            {
                ...
                if (!frame) {
                    cond_var_.wait(auto_lock);//cond_var_.wait(auto_lock)：如果没有帧可用，线程进入等待状态，释放锁，直到被通知。
                    continue;
                }
            }
...
void X264EncoderFilter::OnNewMediaFrame(std::shared_ptr<MediaFrame> frame) {
    ...
    cond_var_.notify_one();//cond_var_.notify_one()：通知一个等待的消费者线程，表示有新数据可用。
}
    ...
    std::shared_ptr<MediaFrame>& out_frame) 
{
    ...
    int nal_num;
    x264_nal_t* nal_out; //编码后输出的 NALU（网络抽象层单元）数组。
    x264_picture_t pic_out;//编码后的视频帧信息。
    //x264_：x264 编码器的实例。
    ...
    int data_size = 0;//最终输出数据的大小。
    std::vector<x264_nal_t> nals;//存储需要处理的 NALU。
    bool idr = false;
    int sps_length = 0;
    char sps[1024] = { 0 };//存储 SPS（序列参数集）数据。
    int pps_length = 0;
    char pps[1024] = { 0 };//存储 PPS（图像参数集）数据。
    ...
    //如果包含 IDR 帧，将 SPS 和 PPS 数据的大小加入到总数据大小中。
    if (idr) {
        data_size += (sps_length + pps_length);
    }

    //创建一个 MediaFrame 对象，用于存储最终的编码数据
    ...
    //将筛选后的 NALU 数据拷贝到输出帧中
    /*如果是 IDR 帧，先拷贝 SPS 和 PPS 数据，再拷贝 IDR 帧数据。
如果是普通切片（P 帧），直接拷贝切片数据。*/
    int data_index = 0;
